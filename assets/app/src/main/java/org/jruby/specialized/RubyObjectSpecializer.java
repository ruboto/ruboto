...
//                    ObjectAllocator allocator = (ObjectAllocator) specialized.getDeclaredClasses()[0].newInstance();
                    ObjectAllocator allocator = (ObjectAllocator) LOADER.loadClass(clsPath.replace('/', '.') + "Allocator").newInstance();
    ...
    private static Class generateInternal(int size, final String clsPath) {
        ...
        Class allocatorClass = defineClass(jiteClass.getChildClasses().get(0));
    ...
    private static Class defineClass(JiteClass jiteClass) {
        return LOADER.defineClass(classNameFromJiteClass(jiteClass), jiteClass.toBytes(JDKVersion.V1_7));
    }
